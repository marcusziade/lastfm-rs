...
mod tests {
    ...
    fn test_cli_error_messages() {
        ...
        assert_eq!(invalid_command.to_string(), "Invalid command: unknown.command");
        ...
        assert_eq!(validation_error.to_string(), "Validation error: Artist name required");
    ...
    fn test_from_json_error() {
        ...
        let parse_result: Result<serde_json::Value> = serde_json::from_str(json_str)
            .map_err(|e| e.into());
    ...
    fn test_error_context_preservation() {
        ...
        let validation_error = CliError::validation("Period must be one of: overall, 7day, 1month, 3month, 6month, 12month");
    ...
    fn test_error_conversion_chain() {
        ...
        assert!(validate_limit("abc").unwrap_err().to_string().contains("must be a number"));
        
        assert!(validate_limit("2000").is_err());
        assert!(validate_limit("2000").unwrap_err().to_string().contains("cannot exceed 1000"));
        
        assert!(validate_limit("0").is_err());
        assert!(validate_limit("0").unwrap_err().to_string().contains("must be greater than 0"));
    }
}